/**
 * GET /api/v1/admin/audit-log — accountability log (ADM-5).
 */
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const entityType =
      request.nextUrl.searchParams.get('entityType') ?? undefined
    const entityId = request.nextUrl.searchParams.get('entityId') ?? undefined
    const cursor = request.nextUrl.searchParams.get('cursor')
    const limitRaw = request.nextUrl.searchParams.get('limit')
    const limit = limitRaw ? Number.parseInt(limitRaw, 10) : undefined

    const page = await admin.listAuditLog.execute(actor, {
      actorId,
      entityType,
      entityId,
      cursor,
    ...
    console.error('GET /api/v1/admin/audit-log failed:', error)
